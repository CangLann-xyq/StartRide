...
using StartRide.App.Views.Shell;
...
public partial class App : System.Windows.Application
{
	...
	protected override async void OnStartup(StartupEventArgs e)
	{
		...
		try
		{
			...
			{
				services.AddSingleton<IThirdPartyAccountService>(serviceProvider =>
				{
					try
					{
						if (FrameworkServiceOverrides.CreateWithoutPathProvider(
							    thirdPartyAccountServiceType, serviceProvider) is IThirdPartyAccountService own)
						{
							return own;
						}
						Log.Warning("StartRide: ThirdPartyAccountService 无路径构造不可用，退回框架公开构造。");
					}
					catch (Exception ex)
					{
						Log.Warning(ex, "StartRide: ThirdPartyAccountService 无路径构造失败，退回框架公开构造。");
					}
					return (IThirdPartyAccountService)FrameworkServiceOverrides.CreateWithServiceProvider(
			...
			if (microsoftAccountServiceType != null)
			{
				...
				{
					if (d.ServiceType == typeof(IMicrosoftAccountService)) microsoftFallback = d;
				}
				services.AddSingleton<IMicrosoftAccountService>(serviceProvider =>
				{
					try
					{
						return (IMicrosoftAccountService)FrameworkServiceOverrides.CreateWithServiceProvider(
							microsoftAccountServiceType, serviceProvider);
					}
					catch (Exception ex)
					{
						Log.Warning(ex, "StartRide: MicrosoftAccountService 轻量构造失败，退回框架默认注册。");
						if (microsoftFallback?.ImplementationFactory != null)
							return (IMicrosoftAccountService)microsoftFallback.ImplementationFactory(serviceProvider);
						throw;
					}
				});
			}
