...
{
	internal const string ResourceName = "StartRide.App.Resources.ReferenceProjects.json";
	...
	public EmbeddedInfoReferenceProjectCatalog(ILogger<EmbeddedInfoReferenceProjectCatalog> logger)
		: this(() => typeof(EmbeddedInfoReferenceProjectCatalog).Assembly.GetManifestResourceStream("StartRide.App.Resources.ReferenceProjects.json"), logger)
	{
	}
	...
	private IReadOnlyList<InfoReferenceProjectItem> LoadProjects()
	{
		try
		{
			using Stream utf8Json = resourceStreamFactory() ?? throw new InvalidDataException("Embedded resource 'StartRide.App.Resources.ReferenceProjects.json' was not found.");
